...
// Bilinear reconstruction, preserving texel fetch at native resolution.
vec3 fetch_bilinear(sampler2D img, ivec2 ipos, vec2 tc)
{
  return (textureSize(img, 0) == imageSize(img_out)) ? texelFetch(img, ipos, 0).rgb : texture(img, tc).rgb;
}
...
vec3 fetch_coupler_subtracted_exposure(ivec2 ipos, sampler2D img_coupler)
{
  ...
  if (params.couplers > 0) coupler = coupler_saturate_post_blur(texture(img_coupler, tc).rgb);
